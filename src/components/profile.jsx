...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    fetchProfile(token)
      .then((data) => {
        ...
        return Promise.all([
          fetchSurveyHistory(token),
          fetchRetakeRequests(token),
          fetchAppointments(token),
          fetchFeedbackByUser(userId, token),
          fetchConsultants(token),
          fetchCampaignSubmissions(userId, token),
        ]);
      })
        ...
        if (userData.roleName === "CONSULTANT") {
          return fetchConsultants(token)
            ...
            .then((matched) => {
              ...
              return Promise.all([
                fetchConsultantSessions(matched.consultantId, token),
  ...
  const handleThanhToanNgay = (courseId, amount) => {
    ...
    createPayment(courseId, profile.userId, token)
      .then((payment) => {
        navigate("/payment-process", {
          state: {
            courseId,
  ...
  return (
    <div className="profile-container">
      {loading ? (
                    ...
                    <ul>
                      {myCourses.map((course) => (
                        <li key={course.courseId} className="course-item">
                          ...
                          <p>
                            <strong>👨‍⚕️ Tư vấn viên:</strong>{" "}
                            {course.consultant?.name || "Không rõ"} (
                            {course.consultant?.email || "N/A"})
                          </p>
        ...
        <p>Không thể tải thông tin cá nhân. Vui lòng đăng nhập lại.</p>
      )}
    </div>
  );
}