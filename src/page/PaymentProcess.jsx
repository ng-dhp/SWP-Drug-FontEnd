...
export default function PaymentProcess() {
  ...
  const [amountFromAPI, setAmountFromAPI] = useState(null);
  ...
  // ✅ Lấy trạng thái thanh toán & số tiền từ API
  useEffect(() => {
    ...
    fetch("http://localhost:8080/api/v1.0/payments/all", {
      ...
      .then((data) => {
        ...
        // ✅ Tìm giao dịch khớp userId và courseId
        ...
        if (payment) {
          ...
          setAmountFromAPI(payment.amount); // ✅ Lưu số tiền từ API
  ...
  return (
    <div className="payment-container">
      ...
      <div className="payment-info">
        <p>
          👤 Học viên: <strong>{fullName || "..."}</strong>
        </p>
        <p>
          📘 Khóa học: <strong>{tenKhoaHoc || "..."}</strong>
        </p>
        <p>
          💵 Số tiền:{" "}
          <strong>
            {amountFromAPI ? `${amountFromAPI.toLocaleString()} VNĐ` : "..."}
          </strong>
        </p>
        <p
      ...
      <img
        src={`https://img.vietqr.io/image/TPB-0339604456-compact.png?amount=${
          amountFromAPI || 0
        }&addInfo=THANH%20TOAN%20KHOA%20HOC&accountName=${encodeURIComponent(
